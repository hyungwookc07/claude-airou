...
이 레포는 러스트 학습 교재로 꽤 유리한 조건을 갖고 있다: **같은 프로그램이 Swift(`Sources/`)와
러스트(`rust/src/`)로 나란히 존재**한다. 이미 아는 쪽(동작)을 기준으로 낯선 쪽(문법·개념)을
...
1. 위 순서로 하루 한 모듈씩, 항상 Swift 원본과 나란히 읽는다.
