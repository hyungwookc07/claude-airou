---
name: hatch-pet
description: Hatch a new custom pet for claude-pet (the Codex-style desktop pet for Claude Code). Use when the user asks to create, design, hatch, or edit a claude-pet pet/companion/creature (e.g. "/hatch-pet a sleepy axolotl", "make me a pet that looks like a goblin", "change my pet's colors"). Produces a validated pixel-art JSON in ~/.claude-pet/pets/ and shows the rendered sprite sheet.
---

# Hatch a claude-pet pet
...
`claude-pet` binary validates and renders it.
...
3. **Design the sprite** following the format and design rules below. Start from the
   ...
   jump.
4. **Write the file** to `~/.claude-pet/pets/<id>.json` (create the folder if needed).
5. **Validate**: `claude-pet validate ~/.claude-pet/pets/<id>.json`. Fix every error
...
6. **Look at it**: `claude-pet render <id> --out /tmp/claude-pet-render-<id> --scale 10` and
...
7. **Hand over**: tell the user the pet is available in the menu bar 🐾 → Pet → `<name>`
   ...
   If `claude-pet` is not on PATH, use `~/.local/bin/claude-pet` or the repo's
   `.build/release/claude-pet`.
...
Rules enforced by `claude-pet validate`:
